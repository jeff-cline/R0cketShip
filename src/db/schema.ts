...
export const userRole = pgEnum("user_role", ["god", "manager", "customer", "agent", "partner", "sales_manager", "bd_partner"]);
...
export const platformSettings = pgTable("platform_settings", {
  ...
  investorReferralFee: numeric("investor_referral_fee").notNull().default("250"), // flat one-time BD investor-referral fee
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});

// ── Business Development partner program (unified into the affiliate/referral system) ──
export const bdTier = pgEnum("bd_tier", ["manager", "vp"]);

export const bdPartners = pgTable("bd_partners", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: uuid("user_id").notNull().references(() => users.id, { onDelete: "cascade" }).unique(),
  tier: bdTier("tier").notNull().default("manager"),
  track: text("track").notNull().default("both"), // clients | investors | both
  sponsorUserId: uuid("sponsor_user_id"), // the VP who recruited this partner (downline)
  slug: text("slug").notNull().unique(), // firstname-lastname → r0cketship.com/opportunity/<slug>
  firstName: text("first_name").notNull(),
  lastName: text("last_name").notNull(),
  city: text("city"),
  state: text("state"),
  zip: text("zip"),
  title: text("title").notNull().default("Business Development"),
  form1099Url: text("form_1099_url"),
  videoWatchedAt: timestamp("video_watched_at"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
});

// Investor-opportunity leads (from /opportunity + /opportunity/<slug>). PII is god-only; partners see name + timestamp.
export const investorLeads = pgTable("investor_leads", {
  id: uuid("id").primaryKey().defaultRandom(),
  referredByUserId: uuid("referred_by_user_id"), // the BD partner who referred (via slug); null if organic
  slug: text("slug"), // partner slug used, if any
  firstName: text("first_name"),
  lastName: text("last_name"),
  email: text("email"),
  phone: text("phone"),
...
// Flat one-time investor referral fees (not a percentage). Accrue on form submit; god can void.
export const bdReferralFees = pgTable("bd_referral_fees", {
  id: uuid("id").primaryKey().defaultRandom(),
  partnerUserId: uuid("partner_user_id").notNull().references(() => users.id),
  investorLeadId: uuid("investor_lead_id").notNull().references(() => investorLeads.id),
  amount: numeric("amount").notNull(),
  status: commissionStatus("status").notNull().default("accrued"), // accrued | owed | paid | void
  createdAt: timestamp("created_at").notNull().defaultNow(),
});
